...
(function ($) {
	...
	function SlickGrid(container, data, columns, options) {
		...
		function getHeadersWidth() {
			// FreezeColumns - If numberOfColumnsToFreeze > 0, calculate frozen and nonFrozen widths
		...
		function getCanvasWidth() {
			...
			while (i--) {
				if (i < numberOfColumnsToFreeze) {
					rowWidth_0 += columns[i].width;
				} else {
					rowWidth_1 += columns[i].width;
				}
			}
		...
		function updateCanvasWidth(forceColumnWidthsUpdate) {
			...
			if (canvasWidth_1 != oldCanvasWidth_1) {
			...
			if (canvasWidth_1 != oldCanvasWidth_1 || forceColumnWidthsUpdate) {
				applyColumnWidths();
			}
		}
		...
		function updateColumnHeader(columnId, title, toolTip) {
			...
			var $header = $headers_1.children().eq(idx);
			if ($header) {
				if (title !== undefined) {
		...
		function setupColumnResize() {
			...
			//TODO: SFA: Dont just read from nonfrozen, incorperate frozen too.
			columnElements = $headers_1.children();
		...
		function applyColumnHeaderWidths() {
			if (!initialized) { return; }
			var h;
			for (var i = 0, headers = $headers_1.children(), ii = headers.length; i < ii; i++) {
				h = $(headers[i]);
		...
		function setSortColumns(cols) {
			...
			var headerColumnEls = $headers_1.children();
			headerColumnEls
		...
		function renderRows(range) {
            ...
            // FreezeColumns - Set stringArray to be a nested object for the frozen and nonFrozen sides
            var stringArray = {"frozen" : [],
                               "nonFrozen" : []};
			var numberOfColumnsToFreeze = options.numberOfColumnsToFreeze;
